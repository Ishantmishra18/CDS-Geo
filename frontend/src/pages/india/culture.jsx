...
import { cultureList } from "../../data/india.js";
...
const Parks = () => {
  ...
  useEffect(() => {
    ...
    const shuffled = [...cultureList].sort(() => Math.random() - 0.5);
    setParks(shuffled);
  ...
  const nextQuestion = () => {
    ...
    if (nextIndex < parks.length) {
      ...
      const reshuffled = [...cultureList].sort(() => Math.random() - 0.5);
      setParks(reshuffled);
      setIndex(0);
  ...
  return (
    <div className="flex flex-col md:flex-row p-4 gap-6">
      ...
      <AboutCard
        img={question?.image}
        question={"this " + question?.type + " belongs to which state"}
        keyword={question?.name}
        description={question?.description}
      >
        <div className="absolut bottom-10 left-5 h-16 w-16">
          <img src={`/stamp/${question?.type}.png`} alt="" className="w-full h-full object cover"/>
        </div>
      </AboutCard>
